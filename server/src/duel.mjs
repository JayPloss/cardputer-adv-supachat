...
export function normalizeSpell(value) {
  ...
  if (normalized === 'sectumsempra' || normalized === 'sectumsempra') return 'sectumsempra';
...
export function resolveRound(firstSpell, secondSpell, firstProtegoStreak = 0, secondProtegoStreak = 0) {
  ...
  if (firstForfeit || secondForfeit) return {
    winner: firstForfeit === secondForfeit ? null : firstForfeit ? 'second' : 'first', reason: firstForfeit === secondForfeit ? 'double_protego_forfeit' : 'protego_forfeit',
    firstProtegoStreak: firstForfeit ? 0 : nextFirstStreak, secondProtegoStreak: secondForfeit ? 0 : nextSecondStreak,
  };
  if (first === 'protego' || second === 'protego' || first === second) return {winner: null, reason: first === second ? 'same_spell' : 'protego', firstProtegoStreak: nextFirstStreak, secondProtegoStreak: nextSecondStreak};
