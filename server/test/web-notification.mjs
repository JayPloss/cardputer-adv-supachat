...
await page.addInitScript(() => {
  ...
  window.fetch = async (url) => ({ ok:true, status:200, json:async () => String(url).includes('presence') ? {presence:[]} : {messages:[]}, arrayBuffer:async () => new ArrayBuffer(0) });
...
await page.evaluate(() => window.testEvents.emit('message', {id:77, author_id:'albie', author_name:'Albie', type:'text', body:'Ping', created_at:Date.now()}));
