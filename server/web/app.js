...
const memberRows = (members, attribute, protectSelf = false, aliases = false) => members?.length ? members.map((member) => `<div class="group-member"><span>${escapeHtml(member.account_name || member.display_name)} <small>${escapeHtml(member.kind)}</small></span>${aliases ? `<label class="alias-editor">Alias<input data-group-alias="${escapeHtml(member.id)}" value="${escapeHtml(member.display_name === member.account_name ? '' : member.display_name || '')}" placeholder="Use account name"></label>` : ''}<button type="button" ${attribute}="${escapeHtml(member.id)}" ${protectSelf && member.id === currentUser.id ? 'disabled title="You cannot remove yourself"' : ''}>Remove</button></div>`).join('') : '<p class="empty-inline">No members.</p>';
async function loadAdminData(selectedRoom = manageRoom.value, selectedUserGroup = manageUserGroup.value) {
  ...
  userGroupMembers.innerHTML = memberRows(userGroup?.members, 'data-remove-group-user', false, true);
...
userGroupMembers.addEventListener('change', async (event) => { const input = event.target.closest('[data-group-alias]'); if (!input) return; input.disabled = true; try { await api(`api/admin/groups/${encodeURIComponent(manageUserGroup.value)}/members/${encodeURIComponent(input.dataset.groupAlias)}`, {method:'PATCH',body:JSON.stringify({display_name:input.value.trim()})}); userGroupState.textContent = 'Alias saved.'; await loadAdminData(manageRoom.value, manageUserGroup.value); } catch { userGroupState.textContent = 'Could not save that alias.'; input.disabled = false; } });
