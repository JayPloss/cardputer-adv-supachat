...
import {SupaChatState,menuItems,changelog} from './state.mjs';
...
s.menu();s.menuSelections[0]=3;s.enter();assert.equal(s.screen,'walkie');s.holdSpace();s.tick(1200);s.releaseSpace();assert.equal(s.clipReady,true);s.enter();assert.equal(s.replayAudible,true);
...
const primaryArrows=new SupaChatState();primaryArrows.menu();assert.equal(primaryArrows.physicalKey('down','.'),'navigation');assert.equal(primaryArrows.menuSelections[0],1);assert.equal(primaryArrows.physicalKey('down','.',{fn:true}),'ignored');assert.equal(primaryArrows.menuSelections[0],1);primaryArrows.right();assert.equal(primaryArrows.menuPage,1);primaryArrows.down();primaryArrows.left();assert.equal(primaryArrows.menuPage,0);primaryArrows.right();assert.equal(primaryArrows.menuSelections[1],1);
...
const changes=new SupaChatState();changes.open('CHANGELOG');assert.equal(changes.screen,'changelog');changes.down();assert.equal(changes.changelogSelection,1);changes.enter();assert.equal(changes.screen,'menu');
s.sync('io');assert.equal(s.network,'SYNC IO -1');s.sync('ok');assert.equal(s.network,'SYNCED');for(const item of menuItems){s.open(item);assert.ok(['chat','rooms','walkie','volume','language','networks','status','changelog','menu'].includes(s.screen)||item==='SYNC NOW')}
