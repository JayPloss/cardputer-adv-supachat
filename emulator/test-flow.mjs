...
s.right();assert.equal(s.currentRoom,'K-BUDS');assert.equal(s.roomNew[1],false);s.left();assert.equal(s.currentRoom,'Family');s.menu();assert.equal(s.screen,'menu');s.down();assert.equal(s.menuSelection,1);s.enter();assert.equal(s.screen,'rooms');s.down();s.enter();assert.equal(s.currentRoom,'K-BUDS');
