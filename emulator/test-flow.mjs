...
s.sync('io');assert.equal(s.network,'SYNC IO -1');s.sync('ok');assert.equal(s.network,'SYNCED');for(const item of menuItems){s.open(item);assert.ok(['chat','rooms','walkie','volume','language','networks','status'].includes(s.screen)||item==='SYNC NOW')}
...
const pin=firmware.match(/kTlsFingerprint\[\] = "([0-9A-F ]+)"/)[1].replaceAll(' ','');assert.equal(pin.length,64);assert.match(firmware,/textEntryScreen \? keys\.fn : !keys\.fn/);assert.match(firmware,/menuSelection = \(menuSelection \+ 1\) % 8/);assert.match(firmware,/kEspNowEnabled = true/);
