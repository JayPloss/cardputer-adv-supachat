...
s.left();s.open('CHAT');assert.equal(s.type('?',{shift:true}),'punctuation');assert.equal(s.draft,'?');const notices=s.notifications;s.receive('Papa','Dinner!');assert.equal(s.notifications,notices+1);
s.acceptDuel('Papa');s.screen='duel';s.up();assert.equal(s.duelSelection,0);s.enter();assert.equal(s.duel.choice,'protego');assert.equal(s.cast('langlock'),false);assert.equal(s.completeDuel(true),true);s.enter();assert.equal(s.duel,null);assert.equal(s.reply('On it'),true);assert.ok(s.messages.at(-1).replyTo);
s.sync('io');assert.equal(s.network,'SYNC IO -1');s.sync('ok');assert.equal(s.network,'SYNCED');for(const item of menuItems){s.open(item);assert.ok(['chat','duel','rooms','walkie','volume','networks','status'].includes(s.screen)||item==='SYNC NOW')}
...
assert.match(firmware,/drawDuelOption\(84,27,72,"PRO","TEGO",0/);assert.match(firmware,/drawDuelOption\(4,57,83,"LEVI","CORP",3/);assert.match(firmware,/duelActionPending="acknowledge"/);assert.match(firmware,/duelActionPending="accept"/);assert.match(firmware,/reactionPendingMessageId/);assert.match(firmware,/reply_to_id/);assert.match(firmware,/api\/typing/);assert.match(firmware,/SLOW DOWN/);
const pin=firmware.match(/kTlsFingerprint\[\] = "([0-9A-F ]+)"/)[1].replaceAll(' ','');assert.equal(pin.length,64);assert.match(firmware,/navigationChord = !\(keys\.shift \|\| keys\.ctrl/);assert.match(firmware,/menuSelection = \(menuSelection \+ 1\) % 4/);assert.match(firmware,/menuPage = \(menuPage \+ 1\) % 2/);assert.match(firmware,/kEspNowEnabled = false/);
