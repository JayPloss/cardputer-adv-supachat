...
s.acceptDuel('Papa');assert.equal(s.cast('protego'),true);assert.equal(s.cast('langlock'),false);assert.equal(s.reply('On it'),true);assert.ok(s.messages.at(-1).replyTo);
...
assert.match(firmware,/DUEL 1:P 2:S 3:L 4:G/);assert.match(firmware,/reactionPendingMessageId/);assert.match(firmware,/reply_to_id/);assert.match(firmware,/api\/typing/);assert.match(firmware,/SLOW DOWN/);
